...
Add the plugin **pinned to the tag your course specifies** (examples below use `v0.3.0`). Pinning the tag is required: it ships the course's public key, and its source hash is what the analyzer recognizes as an approved build.

**lazy.nvim** — add to your plugin spec:

```lua
{ "ProvenanceTools/provenance-neovim-recorder", version = "v0.3.0", lazy = false }
```

**packer.nvim:**

```lua
use({ "ProvenanceTools/provenance-neovim-recorder", tag = "v0.3.0" })
```
